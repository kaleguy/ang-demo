(function () {
  ...
  if (typeof require == 'function') {
    QUnit.test('noConflict (node vm)', function (assert) {
      ...
      fs.readFile(filename, function (err, content) {
        var sandbox = vm.createScript(content + 'this.underscore = this._.noConflict();', filename);
  ...
  QUnit.test('template', function (assert) {
    ...
    var fancyTemplate = _.template('<ul><% ' + '  for (var key in people) { ' + '%><li><%= people[key] %></li><% } %></ul>');
